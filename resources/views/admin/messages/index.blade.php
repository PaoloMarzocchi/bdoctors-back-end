...
@section('content')
    ...
    <div class="container my-3">
        ...
        <div class="wrapper bg_dark_transparent w-100 p-4 mb-4 shadow rounded-lg">
            ...
            <h4 class="mb-4">Hi Dr. <span class="my_primary text-decoration-underline">{{ $doctor->surname }}</span>, you
                have
                <strong class="my_primary">{{ $messageNumber }}</strong> messages.
            </h4>
            ...
            @if ($messages)
                <div class="table-responsive">
                    <table class="table border table-light table-striped table-bordered table-hover text-center">
                        <thead>
                            <tr>
                                <th class="text-start" scope="col">Sender</th>
                        ...
                        <tbody>
                            @foreach ($messages as $message)
                                <tr class="">
                                    <td class="text-start" scope="row">{{ $message->sender_first_name }}
                                    ...
                                    <td>
                                        {{-- {{ $message->created_at }} --}}
                                        {{ $message->formattedDateWithHour($message->created_at) }}
                                    </td>
